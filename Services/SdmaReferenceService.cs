...
namespace SdmaConnector.Services
{
    public class SdmaReferenceService
    {
        private const string FIELD_PREFIX = "Sdma_";
        
        public static readonly List<string> SdmaFieldNames = new List<string>
        {
            $"{FIELD_PREFIX}FileName",
            $"{FIELD_PREFIX}ProjectId",
            $"{FIELD_PREFIX}ProjectName", 
        ...
        /// <summary>
        /// Checks if the specified table/feature class has SDMA reference fields
        /// </summary>
        public static async Task<bool> HasSdmaFields(MapMember mapMember)
        {
            return await QueuedTask.Run(() =>
            {
                try
                {
                    ...
                    // Check if all SDMA fields exist
                    return SdmaFieldNames.All(fieldName => existingFields.Contains(fieldName));
                }
                catch (Exception ex)
                {
                    System.Diagnostics.Debug.WriteLine($"Error checking SDMA fields: {ex.Message}");
                    return false;
                }
            });
        }
        ...
        /// <summary>
        /// Adds SDMA reference fields to the specified table/feature class
        /// </summary>
        public static async Task<bool> AddSdmaFields(MapMember mapMember)
        {
            return await QueuedTask.Run(() =>
            {
                try
                {
                    ...
                    using (var geodatabase = table.GetDatastore() as Geodatabase)
                    {
                        ...
                        var fieldDescriptions = new List<DDLFieldDescription>
                        {
                            ...
                            new DDLFieldDescription($"{FIELD_PREFIX}ProjectId", FieldType.String) { Length = 255 },
                            new DDLFieldDescription($"{FIELD_PREFIX}ProjectName", FieldType.String) { Length = 255 },
                            ...
                            new DDLFieldDescription($"{FIELD_PREFIX}S3Path", FieldType.String) { Length = 1000 },
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"Error adding SDMA fields: {ex.Message}");
                    MessageBox.Show($"Error adding SDMA fields: {ex.Message}", "Error", System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.Error);
                    return false;
                }
            });
        }
        ...
        /// </summary>
        public static async Task AddReferenceToSelectedFeaturesAsync(SdmaFileItem fileItem)
        {
            try
            {
                // Get the active map
                ...
                // Check if SDMA fields exist
                var hasFields = await HasSdmaFields(targetMapMember);
                
                if (!hasFields)
                {
                    var result = MessageBox.Show(
                        $"The layer '{targetMapMember.Name}' does not have SDMA reference fields.\n\nWould you like to add them now?",
                        "Add SDMA Fields",
                    ...
                    {
                        var fieldsAdded = await AddSdmaFields(targetMapMember);
                        if (!fieldsAdded)
                        {
                            MessageBox.Show("Failed to add SDMA fields to the layer.", "Error",
                                System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.Error);
                            return;
                        }
                        
                        MessageBox.Show("SDMA fields have been added successfully!", "Success",
                            System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.Information);
                    }
                    else
                    {
                ...
                var confirmResult = MessageBox.Show(
                    $"Add SDMA file reference '{fileItem.Name}' to {selectionCount} selected feature(s) in '{targetMapMember.Name}'?",
                ...
                {
                    var success = await PopulateSdmaReference(targetMapMember, fileItem);
                    
                    if (success)
                    {
                        MessageBox.Show($"Successfully added SDMA file reference to {selectionCount} feature(s)!", "Success",
                            System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.Information);
                    }
                    else
                    {
                        MessageBox.Show("Failed to add SDMA file reference.", "Error",
                            System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.Error);
                    }
                }
            }
        ...
        /// <summary>
        /// Populates SDMA reference data for selected features/rows
        /// </summary>
        public static async Task<bool> PopulateSdmaReference(MapMember mapMember, SdmaFileItem fileItem)
        {
            return await QueuedTask.Run(() =>
            {
                try
                {
                    ...
                    var fileLink = CreateSdmaFileUrl(fileItem);
                    var file = fileItem.GetFile();
                    
                    // Create S3 path from file key (since S3Uri doesn't exist)
                    var s3Path = $"s3://sdma-bucket/{file.Key}"; // Adjust bucket name as needed
                    
                    // Prepare the attribute values
                    var attributes = new Dictionary<string, object>
                    {
                        [$"{FIELD_PREFIX}FileName"] = fileItem.Name,
                        [$"{FIELD_PREFIX}ProjectId"] = fileItem.ProjectId,
                        [$"{FIELD_PREFIX}ProjectName"] = fileItem.ProjectName,
                        [$"{FIELD_PREFIX}AssetId"] = fileItem.AssetId,
                        [$"{FIELD_PREFIX}AssetName"] = fileItem.AssetName,
                        [$"{FIELD_PREFIX}FileLink"] = fileLink,
                        [$"{FIELD_PREFIX}S3Path"] = s3Path,
                    ...
                    using (var rowCursor = table.Search(queryFilter))
                    {
                        ...
                        editOperation.Name = "Add SDMA File Reference";
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"Error populating SDMA reference: {ex.Message}");
                    MessageBox.Show($"Error populating SDMA reference: {ex.Message}", "Error", 
                        System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.Error);
                    return false;
                }
            });
        ...
        private static string CreateSdmaFileUrl(SdmaFileItem fileItem)
        {
            var fileType = GetFileType(fileItem.FilePath);
            var file = fileItem.GetFile();
            var s3Path = $"s3://sdma-bucket/{file.Key}"; // Adjust bucket name as needed
            var encodedS3Path = Uri.EscapeDataString(s3Path);
            
            return $"sdma-file://{Uri.EscapeDataString(fileItem.ProjectId)}/{Uri.EscapeDataString(fileItem.AssetId)}/{Uri.EscapeDataString(fileItem.FilePath)}?type={fileType}&s3path={encodedS3Path}";
        }
