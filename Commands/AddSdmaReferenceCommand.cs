...
{
    internal class AddSdmaReferenceCommand : Button
    {
        private SdmaFileItem _fileItem;

        public AddSdmaReferenceCommand()
        {
        }

        public void SetFileItem(SdmaFileItem fileItem)
        {
            _fileItem = fileItem;
        }
        ...
        protected override async void OnClick()
        {
            if (_fileItem == null)
            {
                MessageBox.Show("No SDMA file selected.", "Error", 
        ...
        private async Task AddReferenceToSelectedFeatures()
        {
            try
            {
                ...
                var hasFields = await SdmaReferenceService.HasSdmaFields(targetMapMember);
                ...
                if (!hasFields)
                {
                    ...
                    {
                        var fieldsAdded = await SdmaReferenceService.AddSdmaFields(targetMapMember);
                        if (!fieldsAdded)
                        {
                ...
                var confirmResult = MessageBox.Show(
                    $"Add SDMA file reference '{_fileItem.Name}' to {selectionCount} selected feature(s) in '{targetMapMember.Name}'?",
                ...
                {
                    var success = await SdmaReferenceService.PopulateSdmaReference(targetMapMember, _fileItem);
